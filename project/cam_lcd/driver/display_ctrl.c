...
int DisplayStart(DisplayCtrl *dispPtr)
{
	...
	{
		xil_printf("display is running\n");
		return XST_SUCCESS;
	}


	/*
	 * 1. Calculate the PLL divider parameters based on the required pixel clock frequency.
	 * 2. Write to the PLL dynamic configuration registers to configure it with the calculated parameters.
	 * 3. Enable the dynamically generated clock
	 */
	xil_printf("set dyn clk again\n");
	if (DDynClk_SetRate(&dynClk, (uint32_t)dispPtr->vMode.freq) == XST_FAILURE)
	{
	 ...
	 */
	dispPtr->pxlFreq = (double)dynClk.freq;
...
**
*/
  int DisplayInitialize(DisplayCtrl *dispPtr, uint16_t vtcId, uint16_t dynClkId)
{
	int Status;
	...
	/*
	 * 0. Initialize Dynamic Clock device
	 * 1. Calculate the PLL divider parameters based on the required pixel clock frequency.
	 * 2. Write to the PLL dynamic configuration registers to configure it with the calculated parameters.
	 * 3. Enable the dynamically generated clock
	 */
	...
	Status = DDynClk_CfgInitialize(&(dispPtr->dynClk), dynClkConfig, dynClkConfig->BaseAddress);
	if (Status != (XST_SUCCESS)) {
		return (XST_FAILURE);
	}

	if (DDynClk_SetRate(&dispPtr->dynClk, (uint32_t)dispPtr->vMode.freq) == XST_FAILURE)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Error calculating CLK register values\n\r");
		return XST_FAILURE;
	}
	 ...
	 * exactly generate the desired pixel clock, so this may differ from vMode.freq.
	 */
	dispPtr->pxlFreq = (double)(dispPtr->dynClk.freq);
